...
predict = np.zeros(shape=[count, size * 4],dtype = np.double)
...
path = 'saver%d-%d-%d/' % (size,count,node)
...
def predictArray(arr):
    for i in range(count-50):
        array1 = arr[i+15:i+size+15]
        array1.shape = (size * 4)

        predict[i] = array1
    return predict

predict = predictArray(arr)
result = sess.run(y_conv, feed_dict={x: predict});
plt.plot(range(count),result, 'g', label='broadcast')
